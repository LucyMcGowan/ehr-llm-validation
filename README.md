On Using Large Language Models to Enhance Clinically-Driven Missing Data
Recovery Algorithms in Electronic Health Records
================
...
1.  [`ellmer`](https://ellmer.tidyverse.org/): to call large language
    models (LLM)
2.  [`dplyr`](https://dplyr.tidyverse.org/): to manipulate data and
    merge
3.  [`stringr`](https://stringr.tidyverse.org/): to clean up strings for
    merging
4.  [`tidyr`](https://tidyr.tidyverse.org/): to split strings and
    reformat roadmaps
...
First, we provided the roadmap’s structure as a dataframe “tool” that
the LLM could access and modify when prompted. Specifically, we created
a function that takes search terms for the ALI components (as vectors)
and combines them following the clinical roadmap’s structure (a
$10 \times 2$ dataframe with a rows per component and a column for
search terms).
...
Next, we defined a tool that built on this function by adding a
...
Then, through the `ellmer` package’s tool functionality, we registered
this tool (i.e., gave Gemini access to use it) and prompted the LLM to
generate relevant terms for each ALI component.
...
Finally, we essentially requested updates to this roadmap dataframe from
the LLM, which `ellmer` executed within our `R` session based on either
Prompt 1 or 2 from the main text. In this way, we obtained a
programmatically-updated roadmap object that could be immediately used
in our missing data recovery algorithm pipeline.
...
Building on the same `make_data` function for the roadmap’s structure,
the LLM prompt was modified slightly to instruct the LLM to include the
examples from the clinicians’ original roadmap (Table 1 in the paper) in
each iteration. The rest of the code proceeds as with **LLM
(Baseline)**.
...
We begin by reading in the ICD-10 codes with their diagnosis
descriptions, which are archived on this GitHub. Then, we do minor
string cleaning on the diagnosis descriptions (making them all capital
letters and trimming potential whitespace) to make matching easier.
...
Next, we read in a 10 x 2 dataframe for our chosen roadmap. Here, we
demonstrate with the LLM (context) roadmap. After reading it in, we
again clean the diagnosis descriptions.
...
```
    ...
    ## 1                 Renal failure; renal insufficiency; ESRD; end-stage renal disease; kidney failure; chronic kidney disease; CKD
    ## 2                                                                              Obesity; morbid obesity; overweight; BMI elevated
    ## 3                    Hypertension; high blood pressure; HTN; essential hypertension; secondary hypertension; hypertensive crisis
    ## 4                    Hypertension; high blood pressure; HTN; essential hypertension; secondary hypertension; hypertensive crisis
    ## 5 Diabetes; impaired glucose; Type 1 diabetes; Type 2 diabetes; diabetes mellitus; DM; hyperglycemia; diabetic ketoacidosis; DKA
    ## 6                                     Hypercholesterolemia; high cholesterol; dyslipidemia; hyperlipidemia; elevated cholesterol
...
```
    ...
    ## 1       CREAT_C           RENAL.*FAILURE
    ## 2       CREAT_C     RENAL.*INSUFFICIENCY
    ## 3       CREAT_C                     ESRD
    ## 4       CREAT_C ENDSTAGE.*RENAL.*DISEASE
    ## 5       CREAT_C          KIDNEY.*FAILURE
    ## 6       CREAT_C CHRONIC.*KIDNEY.*DISEASE

Finally, we create a cross-join between the ICD-10 codes and the long
version of the roadmap and check for matches.
...
```
    ...
    ##   DX_CODE                                     DX_DESC Variable_Name
    ## 1    A021                           SALMONELLA SEPSIS           CRP
    ## 2   A0220 LOCALIZED SALMONELLA INFECTION, UNSPECIFIED           CRP
    ## 3   A0229   SALMONELLA WITH OTHER LOCALIZED INFECTION           CRP
    ## 4    A028       OTHER SPECIFIED SALMONELLA INFECTIONS           CRP
    ## 5    A029           SALMONELLA INFECTION, UNSPECIFIED           CRP
    ## 6    A040 ENTEROPATHOGENIC ESCHERICHIA COLI INFECTION           CRP
    ##   If_Missing_Search_For
    ## 1                SEPSIS
    ## 2             INFECTION
    ## 3             INFECTION
...
It’s possible that multiple search terms match the same ICD-10 codes, so
we then group them and summarize all matches per variable per diagnosis.
...
```
    ...
    ## # A tibble: 6 × 4
    ##   Variable_Name DX_CODE DX_DESC                                    matched_terms
    ##   <chr>         <chr>   <chr>                                      <chr>        
    ## 1 A1C           A312    DISSEMINATED MYCOBACTERIUM AVIUM-INTRACEL… DM           
    ## 2 A1C           E0800   DIABETES MELLITUS DUE TO UNDERLYING CONDI… DIABETES; DI…
    ## 3 A1C           E0801   DIABETES MELLITUS DUE TO UNDERLYING CONDI… DIABETES; DI…
    ## 4 A1C           E0810   DIABETES MELLITUS DUE TO UNDERLYING CONDI… DIABETES; DI…
    ## 5 A1C           E0811   DIABETES MELLITUS DUE TO UNDERLYING CONDI… DIABETES; DI…
    ## 6 A1C           E0821   DIABETES MELLITUS DUE TO UNDERLYING CONDI… DIABETES; DI…

Then, we merged the roadmaps into patients’ ICD-10 codes extracted from
