...
# Revised versions: loop over prompt to do 1 time, ask for "ICD-10" codes 
...
extract_terms_nocontext <- function(df, id_value, variable_name) {
...
for (comp in ali_components) {
  all_terms_comp <- do.call(
    rbind,
    Map(extract_terms_nocontext, list(df_nocontext), 1:20, comp)
  ) |>
    pull(term) |>
    unique() |>
    paste(collapse = "; ")
...
super_roadmap |>
  write.csv(
    here::here("data-raw/llm_nocontext_loop_icd10_superset_roadmap.csv"),
    row.names = FALSE
  )
...
## Create function to concatenate all the unique terms across all LLM roadmaps
extract_terms_context <- function(df, id_value, variable_name) {
  terms <- unlist(strsplit(
    paste(
      df |>
        filter(
...
for (comp in ali_components) {
  all_terms_comp <- do.call(
    rbind,
    Map(extract_terms_context, list(df_context), 1:20, comp)
  ) |>
    pull(term) |>
    unique() |>
    paste(collapse = "; ")
...
super_roadmap |>
  write.csv(
    here::here("data-raw/llm_context_loop_icd10_superset_roadmap.csv"),
    row.names = FALSE
  )


# Original versions: prompt to do 20 times, ask for "ICD" codes 
# Make one data frame of the superset of LLM roadmaps
## Load separate dataframes for each LLM roadmap
