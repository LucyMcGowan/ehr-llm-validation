...
roadmap_llm_context_loop_icd10 = df_context |> 
  dplyr::select(id, Variable_Name, If_Missing_Search_For) |>
...
roadmap_llm_nocontext_loop_icd10 = df_nocontext |> 
  dplyr::select(id, Variable_Name, If_Missing_Search_For) |>
  separate_longer_delim(cols = If_Missing_Search_For, delim = ";") |> ## Create separate rows for each variable, keyword combo
  mutate(
    If_Missing_Search_For = toupper(If_Missing_Search_For), ## Convert to all CAPS for easier search
...
matches_llm_context_loop_icd10 = dx_uq |>
  ...
  group_by(id, Variable_Name, DX_CODE, DX_DESC) |>
  summarise(
...
pat_dx_flags_llm_context_loop_icd10 |>
  ...
  write.csv(
    here::here("data-raw/patient_data/dx_llm_context_loop_icd10_superset_roadmap_separate_prompts.csv"),
    row.names = FALSE
  )
pat_dx_flags_nocontext_loop_icd10 |>
  filter(has_match_llm_nocontext_loop_icd10) |>
  write.csv(
    here::here("data-raw/patient_data/dx_llm_nocontext_loop_icd10_superset_roadmap_separate_prompts.csv"),
    row.names = FALSE
  )
...
pat_dx_flags_llm_context_loop_icd10 |>
  filter(!has_match_llm_context_loop_icd10) |>
  write.csv(
    here::here("data-raw/patient_data/dx_llm_context_loop_icd10_superset_roadmap_separate_prompts_unmatched.csv"),
    row.names = FALSE
  )
pat_dx_flags_nocontext_loop_icd10 |>
  filter(!has_match_llm_nocontext_loop_icd10) |>
  write.csv(
    here::here("data-raw/patient_data/dx_llm_nocontext_loop_icd10_superset_roadmap_separate_prompts_unmatched.csv"),
    row.names = FALSE
  )
