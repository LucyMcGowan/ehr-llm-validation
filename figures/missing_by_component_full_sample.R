...
# Count number of originally missing components 
all_data |> 
  filter(DATA == "ALI_COMPONENT") |> 
  pull(COMPONENT) |> 
  table(useNA = "always") ## 4009 missing in extracted EHR data 
all_data |> 
  filter(DATA == "LLM_CONTEXT_CLINICIAN_ALI_COMPONENT") |> 
  pull(COMPONENT) |> 
  table(useNA = "always") ## 3220 missing after recovery algorithm
(4009 - 3220) ## 789 recovered 
(4009 - 3220) / 4009 ## 20% 
...
num_miss |> 
  group_by(DATA) |> 
  summarize(median_nonmiss = median(1000 - NUM_MISSING))
...
num_recov |> 
  filter(DATA == "LLM_CONTEXT_CLINICIAN_ALI_COMPONENT") |> 
  mutate(PROP_RECOVERED = NUM_RECOVERED / NUM_MISSING_EHR) |> 
  arrange(desc(NUM_RECOVERED)) |> 
  head()
